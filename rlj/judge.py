...
class Judge(object):
	...
	def _judge(self, tesk):
		tesk = str(tesk)
		with open('data/' + tesk.join(self.Input.split('#')), 'r') as Input:
			with open('temp/temp.out', 'w') as Output:
				with open('temp/stderr', 'w') as Err:
					begin_time = time.time()
					max_memory = 0
					time_used = 0
					child = subprocess.Popen('temp/prog',
		...
		diff_result = os.system('diff -Z temp/temp.out data/'
