...
class JudgeInfo(object):
    ...
    Info of a judge.
    """
    def __init__(self, status, time_used=None, memory_used=None, returncode=None):
        self.status = status
...
class Compiler(object):
    ...
    def compile(self, source=None):
        ...
        if source is None:
            print('没有源文件！')
            print('请在config.json里设置源文件或执行时指定！')
            exit(1)
...
class Judge(object):
    ...
    def _judge(self, tesk):
        tesk = str(tesk)
        Input = open('data/' + tesk.join(self.Input.split('#')), 'r')
        Output = open('temp/temp.out', 'w')
        Err = open('temp/stderr', 'w')
        begin_time = time.time()
        max_memory = 0
        time_used = 0
        child = subprocess.Popen('temp/prog', stdin=Input, stdout=Output, stderr=Err)
        ps = psutil.Process(child.pid)

        while child.poll() is None:
            mem_info = ps.memory_info()
            memory = float(mem_info.vms + mem_info.rss) / 1048576
        ...
        diff_result = os.system('diff temp/temp.out data/'
            + tesk.join(self.Output.split('#')) + '>> temp/diff_log' + str(tesk))
    ...
    def judge(self):
        ...
        for tesk in self.Num:
            result.append((tesk, self._judge(tesk)))
        return result
